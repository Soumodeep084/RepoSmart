...
export default function AiAnalyzePage() {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await postJson<AiScanResponse>(
        "/api/repo/ai-scan",
        { url: repoUrl },
        { token },
      );
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-10 relative z-10">
            <div className="max-w-3xl mx-auto">
              ...
              >
                ...
                {user ? <span className="text-[#8b949e]">• Signed in as {user.username}</span> : null}
              </div>

              <h1 className="rs-text-glow text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight mt-4">
                AI scan a GitHub repository
              </h1>
              <p className="text-base sm:text-lg text-[#8b949e] mt-3 leading-relaxed">
                Paste a public repository URL to generate an AI report based on GitHub metadata.
              </p>

              <form onSubmit={handleScan} className="mt-8 bg-surface-1 border border-[#30363d] rounded-xl p-4 sm:p-6">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        {isDetectedLicense(result.repo.license) ? (
                          <p className="mt-2 text-sm text-[#c9d1d9] leading-relaxed">
                            Licensed under <span className="font-semibold text-white">{result.repo.license}</span>.
                          </p>
                        ) : (
                          <>
                            <p className="mt-2 text-sm text-[#c9d1d9] leading-relaxed">No license detected.</p>
                            <p className="mt-2 text-xs text-red-400 leading-relaxed">
                              Warning: This repository does not advertise a license. Add a LICENSE file to clarify how others may use,
                      ...
                      <span
                        className={
                          ...
                          (isDetectedLicense(result.repo.license) ? "text-[#c9d1d9]" : "text-red-400")
                        }
                      >
                        {isDetectedLicense(result.repo.license) ? "Licensed" : "Warning"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-surface-1 border border-[#30363d] rounded-xl p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-1 text-white font-semibold text-lg wrap-break-word">{result.repo.fullName}</div>
                    ...
                    <div className="mt-5 bg-surface-2 border border-[#30363d] rounded-lg p-4">
                  ...
                  <div className="bg-surface-1 border border-[#30363d] rounded-xl p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-[#8b949e]">Model: {result.ai.model}</div>
                      </div>
                    </div>
                    <pre className="mt-3 text-sm text-[#c9d1d9] leading-relaxed whitespace-pre-wrap">
                      {result.ai.output}
                    </pre>
                  </div>
                </div>
